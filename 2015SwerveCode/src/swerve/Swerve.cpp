...
void Swerve::Run() {
	/*FrontLeftMod->drive(0.5 * Controller->GetRawAxis(0), 0.50 * Controller->GetRawAxis(1));
	FrontRightMod->drive(0.5 * Controller->GetRawAxis(0), 0.50 * Controller->GetRawAxis(1));
	BackRightMod->drive(0.5 * Controller->GetRawAxis(0), 0.50 * Controller->GetRawAxis(1));
	BackLeftMod->drive(0.5 * Controller->GetRawAxis(0), 0.50 * Controller->GetRawAxis(1));*/
	...
	bool SetSpinMode = fabs(SpinAxis) > 0.25;

	// Get Spin Axis.
	if (SetSpinMode || SpinMode) {
		SpinMode = true;
		FrontLeftMod->AutoDrive(329.30);
		FrontRightMod->AutoDrive(30.70);
		BackLeftMod->AutoDrive(30.70);
		BackRightMod->AutoDrive(329.30);

		if (FrontRightMod->AtAngle() && BackRightMod->AtAngle() && FrontLeftMod->AtAngle() && BackLeftMod->AtAngle() && SetSpinMode) {
			if (COMPETITION_BOT) { // Comp Bot
				BackRightMod->drive(0, -1 * SpinTurtle * SpinAxis);
				BackLeftMod->drive(0, 1 * SpinTurtle * SpinAxis);
			} else if (PRACTICE_BOT) { // Practice Bot
				BackRightMod->drive(0, 1 * SpinTurtle * SpinAxis);
				BackLeftMod->drive(0, -1 * SpinTurtle * SpinAxis);
			}
	...
	if (Controller->GetMagnitude() > 0.05) {
		SpinMode = false;
...
void Swerve::Tune() {
	...
	if (newtime.Get() >= 0.25) {
		...
		if (Controller->GetRawButton(9) == 1) {
			...
			i += 0.000001;
			BackRightMod->Reset();
		} else if (Controller->GetRawButton(3) == 1) {
			i -= 0.00001;
			BackRightMod->Reset();
