...
void Swerve::Run() {
	...
	FrontLeftMod->shittydrive(.5*Controller->GetRawAxis(1));
	FrontLeftMod->shittyangle(.5*Controller->GetRawAxis(0));
	...
	/*if (SetSpinMode || SpinMode) {
		...
		if (FrontRightMod->AtAngle() && BackRightMod->AtAngle() && FrontLeftMod->AtAngle() && BackLeftMod->AtAngle() && SetSpinMode) {
			...
			/*FrontLeftMod->drive(-1 * SpinTurtle * SpinAxis);
			FrontRightMod->drive(SpinTurtle * SpinAxis);
		} else {
	...
	//	FrontLeftMod->drive(DriveTurtle * sign * Magnitude);
	//	FrontRightMod->drive(DriveTurtle * sign * Magnitude);
	//	BackRightMod->drive(DriveTurtle * sign * Magnitude);
	//	BackLeftMod->drive(DriveTurtle * sign * Magnitude);
		/*} else {
		...
		FrontLeftMod->DashboardLoop();
		FrontRightMod->DashboardLoop();
		BackRightMod->DashboardLoop();
		BackLeftMod->DashboardLoop();
	}
